...
- Chat (global and project thread) operations:
  ...
  - Press `F1`, type `/help`, or enter `?` in the prompt to open shell command discovery. Textual shell commands now include `/open`, `/edit`, `/settings`, `/abort`, `/compact`, `/close`, `/queue`, `/exit`, `/tab_left`, `/tab_right`, `/msg_prev`, `/msg_next`, `/msg_first`, `/msg_last`, `/go_to_sidebar`, `/go_to_prompt`, and `/model`; `F5` steers, `F7` delegates, and `ALT+LEFT` / `ALT+RIGHT` switch between open tabs.
  ...
  - Press `Ctrl+G Ctrl+S` to focus the sidebar on the current selection, `Ctrl+G Ctrl+P` to focus the prompt, `Ctrl+G Ctrl+O` or `/model` to focus the provider/model selector, `Ctrl+G Ctrl+R` or use the footer provider summary to open the model providers dialog, `Ctrl+G Ctrl+W` or `/settings` to open workspace settings, `Ctrl+G Ctrl+N` or `/plugins` to open plugin management, `Ctrl+G Ctrl+U` or use the footer usage indicator to open the context/usage popup, and press `Ctrl+G Ctrl+T` or use the thread info icon in the footer to open the selected thread report.
