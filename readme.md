...
- Press `Ctrl+T` or click the thread info icon in the footer to open a popup report for the current thread, with copy-to-markdown, backend storage details when exposed, precise local timestamps, elapsed time, and backend-specific facts such as Codex source/model provider or whether a Copilot session is remote (`CodeAlta`)
