...
- Press `F1`, type `/help`, or enter `?` in the prompt to open shell command discovery; textual shell commands now include `/open`, `/edit`, `/settings`, `/abort`, `/compact`, `/close`, `/queue`, `/exit`, `/tab_left`, `/tab_right`, `/msg_prev`, `/msg_next`, `/msg_first`, `/msg_last`, `/go_to_sidebar`, `/go_to_prompt`, `/model`, and `/models`, while `F5` steers, `F7` delegates, and `ALT+LEFT` / `ALT+RIGHT` switch between open tabs (`CodeAlta`)
