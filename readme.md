...
- Paste images into prompts with `Ctrl+V` when the selected model advertises image input support (or `Ctrl+G Ctrl+V` for the temporary image-only fallback); pasted images open a preview/title dialog, appear as prompt attachment chips, are stored beside the session journal, and remain viewable from user messages in the timeline (`CodeAlta`)
